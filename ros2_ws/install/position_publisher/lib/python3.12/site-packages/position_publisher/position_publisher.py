...
class Vicon2MavlinkBridge(Node):
    ...
    def __init__(self, mavlink_connection_string, use_rt_scheduling=True):
        ...
        #Josh Stuff
        self.create_service(Trigger, 'rc8_low',  self.handle_rc8_low)
        self.create_service(Trigger, 'rc8_mid',  self.handle_rc8_mid)
        self.create_service(Trigger, 'rc8_high', self.handle_rc8_high)

        self.get_logger().info('Services ready: /save_zero_position  /goto_zero_position')

        # Josh's Bo-Schitt
        self.zero_pub = self.create_publisher(PoseStamped, 'zero_position', 10)
        self.rc8_pub = self.create_publisher(Int32, 'rc8_state', 10)
        ...
        while rclpy.ok():
            try:
                msg = self.mavlink_master.recv_match(
                    ...
                    type=['SYSTEM_TIME', 'STATUSTEXT', 'COMMAND_ACK', 'RC_CHANNELS']
                )
                if msg is None:
                    continue
                ...
                if t == 'SYSTEM_TIME':
                    ...
                    raw = msg.chan8_raw
                    value = 1 if raw > 1700 else 0
                    out = Int32()
                    out.data = value
                    self.rc8_pub.publish(out)
            except Exception as e:
    ...
    def handle_save_zero(self, request, response):
        ...
        # Build and publish the PoseStamped
        msg = PoseStamped()
        ...
        # Encode yaw as a quaternion (rotation about Z in NED)
        q = R.from_euler('z', yaw0).as_quat()   # returns [qx, qy, qz, qw]
    ...
    def set_rc8_switch(self, state):
        pwm_map = {'low': 1000, 'mid': 1500, 'high': 1900}
        rc_channel_values = [65535] * 18
        rc_channel_values[7] = pwm_map[state]
        self.mavlink_master.mav.rc_channels_override_send(
            self.mavlink_master.target_system,
            self.mavlink_master.target_component,
            *rc_channel_values
        )
        ...
        msg = Int32()
        msg.data = value
        self.rc8_pub.publish(msg)
        self.get_logger().info(f'RC8 → {state} ({pwm_map[state]} µs)  published {value}')

    def handle_rc8_low(self, request, response):
        self.set_rc8_switch('low')
        response.success, response.message = True, 'RC8 low'
        return response

    def handle_rc8_mid(self, request, response):
        self.set_rc8_switch('mid')
        response.success, response.message = True, 'RC8 mid'
        return response

    def handle_rc8_high(self, request, response):
        self.set_rc8_switch('high')
        response.success, response.message = True, 'RC8 high'
        return response
